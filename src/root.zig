...
pub fn totp(secret: []const u8, now: Io.Timestamp, config: Config) u32 {
    ...
    const hash = hotp(secret, t_input, config.hash_alg);
...
fn hotp(key: []const u8, counter: []const u8, hash_alg: type) [20]u8 {
    const hmac = crypto.auth.hmac.Hmac(hash_alg);
...
test "hotp_values" {
    ...
    const hash_0 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 0 }, crypto.hash.Sha1);
    ...
    const hash_1 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 1 }, crypto.hash.Sha1);
    ...
    const hash_2 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 2 }, crypto.hash.Sha1);
    ...
    const hash_3 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 3 }, crypto.hash.Sha1);
    ...
    const hash_4 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 4 }, crypto.hash.Sha1);
    ...
    const hash_5 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 5 }, crypto.hash.Sha1);
    ...
    const hash_6 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 6 }, crypto.hash.Sha1);
    ...
    const hash_7 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 7 }, crypto.hash.Sha1);
    ...
    const hash_8 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 8 }, crypto.hash.Sha1);
    ...
    const hash_9 = hotp(secret, &[8]u8{ 0, 0, 0, 0, 0, 0, 0, 9 }, crypto.hash.Sha1);
